...
export default async function BuilderPublicPage({ params }: { params: Promise<{ username: string }> }) {
  ...
  if (!profile || !profile.builderProfile) notFound();
  const linkedFounder =
  ...
  return (
    <main className="mx-auto max-w-4xl space-y-6 px-4 py-10">
      <section className="rounded-2xl border border-border/60 bg-card p-6">
        ...
        <p className="mt-2 text-sm text-muted-foreground">{profile.builderProfile.headline ?? profile.builderProfile.bio ?? "No headline provided."}</p>
        ...
        <div className="mt-3 flex items-center gap-2">
          ...
          {showTelegram ? (
            <a
              href={
                normalizeTelegramUrl(founderTelegramValue)!
              }
              target="_blank"
              rel="noreferrer"
