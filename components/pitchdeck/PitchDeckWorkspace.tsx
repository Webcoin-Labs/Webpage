    ...
    <div className="space-y-4">
      <section className="rounded-2xl border border-border/60 bg-card/80 p-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-[11px] uppercase tracking-[0.16em] text-cyan-300">Pitch Deck Workspace</p>
            <h1 className="mt-1 text-xl font-semibold">AI-powered deck diagnosis and improvement</h1>
            <p className="mt-1 text-xs text-muted-foreground">Free: upload + analyze. Pro: rewrite and generate advanced deck versions.</p>
          </div>
      ...
      <section className="grid gap-4 xl:grid-cols-[280px_minmax(0,1fr)_340px]">
        <aside className="sticky top-4 h-fit rounded-2xl border border-border/60 bg-card p-3">
        ...
        <main className="space-y-4 rounded-2xl border border-border/60 bg-card p-4">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="rounded-full border border-border/60 px-2 py-1 text-[10px] text-muted-foreground">
              Process {activeDeck?.processingStatus ?? "N/A"}
            </span>
            <span className="rounded-full border border-border/60 px-2 py-1 text-[10px] text-muted-foreground">
          ...
          {tab === "overview" ? (
            <div className="space-y-4">
              <section className="rounded-xl border border-border/60 bg-background p-4">
                <p className="text-sm font-semibold">Upload deck</p>
                <form
                ...
                >
                  {projects.length > 0 ? (
                    <select name="projectId" className="w-full rounded-md border border-border bg-background px-3 py-2 text-sm" defaultValue="">
                  ...
                  <label className="flex cursor-pointer items-center gap-2 rounded-md border border-dashed border-border/70 px-3 py-3 text-sm text-muted-foreground">
                    <UploadCloud className="h-4 w-4 text-cyan-300" />
                    Upload PDF (PPTX disabled until parser support is production-ready)
              ...
              <section className="rounded-xl border border-border/60 bg-background p-4">
                <p className="text-sm font-semibold">Readiness summary</p>
                {activeReport ? (
                  <div className="mt-3 grid gap-3 sm:grid-cols-3">
                    <p className="rounded-md border border-border/60 px-3 py-2 text-xs text-muted-foreground">
                      Clarity: {activeReport.clarityScore ?? "N/A"}
                    </p>
                    <p className="rounded-md border border-border/60 px-3 py-2 text-xs text-muted-foreground">
                      Completeness: {activeReport.completenessScore ?? "N/A"}
                    </p>
                    <p className="rounded-md border border-border/60 px-3 py-2 text-xs text-muted-foreground">
                      Investor readiness: {activeReport.investorReadinessScore ?? "N/A"}
                    </p>
                  </div>
                ) : (
                  <p className="mt-2 text-sm text-muted-foreground">
                    No analysis report yet. Scores are shown only after real deck parsing and analysis.
                  </p>
                )}
              </section>
            </div>
        ...
        <aside className="space-y-4 rounded-2xl border border-border/60 bg-card p-4">
          <section>
            <p className="text-xs uppercase tracking-[0.14em] text-cyan-300">AI actions</p>
            <div className="mt-2 space-y-2">
              <button
                type="button"
                onClick={onGenerateImprovedVersion}
                disabled={!isPremium || pending || !activeDeck}
                className="w-full rounded-md border border-cyan-500/30 bg-cyan-500/10 px-3 py-2 text-xs text-cyan-200 disabled:cursor-not-allowed disabled:opacity-50"
              >
              ...
              {!isPremium ? (
                <div className="rounded-md border border-amber-500/30 bg-amber-500/10 p-2 text-xs text-amber-200">
                  ...
                  <p className="mt-1 text-amber-100/80">Free plan keeps full upload, analysis, and export access.</p>
                </div>
              ) : null}
            </div>
          </section>
