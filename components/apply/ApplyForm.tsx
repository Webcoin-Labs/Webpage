...
type AppType = "BUILDER_PROGRAM" | "FOUNDER_SUPPORT" | "PARTNER";

export function ApplyForm({
  title,
  description,
  fixedType,
}: { title: string; description: string; fixedType?: AppType }) {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (fixedType) formData.set("type", fixedType);
        ...
        {!fixedType && (
          <div>
            ...
            <div className="space-y-2">
              {[
                { value: "BUILDER_PROGRAM", label: "Builder Program", desc: "Join a builder cohort" },
                { value: "FOUNDER_SUPPORT", label: "Founder Support", desc: "Advisory and support" },
                { value: "PARTNER", label: "Partnership", desc: "Ecosystem or sponsor partnership" },
              ].map((t) => (
                <label key={t.value} className="flex items-start gap-3 p-3 rounded-lg border border-border hover:border-cyan-500/30 cursor-pointer transition-colors has-[:checked]:border-cyan-500/50 has-[:checked]:bg-cyan-500/5">
                  <input type="radio" name="type" value={t.value} className="mt-0.5 accent-cyan-500" />
                  <div>
